...
A random variable is a variable that can take on different values randomly or uncertainly. The values of a random variable are determined by the outcomes of a random process, such as the roll of a dice, the flip of a coin, or the result of a survey.

There are two types of random variables: discrete and continuous. Discrete random variables are variables that can take on a finite or countably infinite number of distinct values. Continuous random variables are variables that can take on any value within a specified range.
