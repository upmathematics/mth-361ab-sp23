...
<font size="4"> **MTH 361 A/B SP23** (University of Portland): [**Lab 3 R Studio Cloud**](https://posit.cloud/spaces/326046/content/5713369){target="_Blank'} </font>
...
You will investigate the ways in which the statistics from a random sample of data can serve as point estimates for population parameters.
...
1.  Use the example code below to create sampling distributions of proportions of *Doesn't benefit* from samples of size 10, 50, and 100. Use 5,000 simulations. What does each observation in the sampling distribution represent? How does the mean, standard error, and shape of the sampling distribution change as the sample size increases? How (if at all) do these values change if you increase the number of simulations? (Include plots in your answer.)

```{r sample-size-effects, fig.show="hide"}
### --------------- Modify Here ---------------
...
### ----- Leave these lines of code as is -----
# perform simulations to create sampling distributions
sample_props <- global_monitor %>% 
  rep_sample_n(size = sample_size, reps = simulations, replace = TRUE) %>%
  count(scientist_work) %>%
  mutate(p_hat = n /sum(n)) %>%
  filter(scientist_work == "Doesn't benefit")

# plot the sampling distribution
plot_sampling <- ggplot(data = sample_props, aes(x = p_hat)) +
  geom_histogram(binwidth = 0.02) +
  labs(
    x = "p_hat (Doesn't benefit)",
    title = "Sampling distribution of p_hat",
...
4.  Of the sampling distributions from the previous two exercises, which has a smaller spread?
...
In order to keep the programming simpler, we are providing the example code below that basically does this for you and created a plot similar to Figure 5.6 on [OpenIntro Statistics, 4th Edition (page 182)](https://www.openintro.org/os).

1.  Given a sample size of 60, 1000 bootstrap samples for each interval, and 50 confidence intervals constructed (copy and modify the example code below), what proportion of your confidence intervals include the true population proportion? Is this proportion exactly equal to the confidence level? If not, explain why. Make sure to include your plot in your answer.

```{r confidence-intervals}
### --------------- Modify Here ---------------
# sample size, number of simulations, number of confidence intervals
sample_size = 20 # number of sample in each bootstrap sample
simulations = 100 # number of bootstrap samples
n_ci = 50 # number of confidence intervals to simulate
cl = 0.95 # the confidence level
### -------------------------------------------
...
# set true p of success
true_p <- 0.62

# a function to create a confidence interval
...
df_ci <- map_dfr(1:n_ci, 
                 store_ci,
                 n = sample_size,
                 reps = simulations, 
                 conf_level = cl,
                 success = success) %>%
          mutate(y_lower = 1:n_ci,
                 y_upper = 1:n_ci,
                 capture_p = ifelse(x_lower < true_p & x_upper > true_p, "Yes", "No"))

# plot confidence intervals
plot_intervals <- ggplot(df_ci) + 
  ...
  geom_vline(xintercept = true_p, color = "darkgray") +
  labs(y = "", 
       x = "Bounds of the confidence interval",
       color = "Does the interval capture the true population proportion?",
       title = paste("cl = ",cl,", # of CIs = ",n_ci)) +
...
3.  Using the example code above, calculate 50 confidence intervals at the confidence level you chose in the previous question, and plot all intervals on one plot, and calculate the proportion of intervals that include the true population proportion.
...
4.  Lastly, try one more (different) confidence level.
    ...
    Finally, use the example code to generate many intervals and calculate the proportion of intervals that are capture the true population proportion.

5.  Using the example code, experiment with different sample sizes and comment on how the widths of intervals change as sample size changes (increases and decreases).
